...
describe('Recintos do Zoologico', () => {
    ...
    test('Não deve encontrar recintos para crocodilo na savana', () => {
        const resultado = new RecintosZoo().analisaRecintos('CROCODILO', 1);
        expect(resultado.erro).toBe("Não há recinto viável");
        expect(resultado.recintosViaveis).toBeFalsy();
    });
    ...
    test('Não deve encontrar recintos para leopardo onde há outros animais', () => {
        const resultado = new RecintosZoo().analisaRecintos('LEOPARDO', 1);
        expect(resultado.erro).toBe("Não há recinto viável");
        expect(resultado.recintosViaveis).toBeFalsy();
    });

    test('Deve encontrar recinto para 3 macacos com espaço extra', () => {
    ...
    test('Macaco sozinho não deve encontrar recintos', () => {
        const resultado = new RecintosZoo().analisaRecintos('MACACO', 1);
        expect(resultado.erro).toBe("Não há recinto viável");
        expect(resultado.recintosViaveis).toBeFalsy();
    });

    test('Deve encontrar recinto para 2 gazelas em bioma savana', () => {
